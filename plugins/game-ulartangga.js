...
// FIX: Panggil default export-nya dulu biar Node nggak marah
...
class SnakeAndLadderGame {
    ...
    async getBoardBuffer() {
        // FIX: Constructor Jimp sesuaikan dengan versi library lu
        // Kalau error 'width is not defined', ganti ke: new Jimp(420, 420, 0xFFFFFFFF)
...
// ... Sisanya (handler) sama kayak kode lu sebelumnya
const handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    switch (args[0]) {
        case 'join':
            if (session?.started) return m.reply('❌ Game udah jalan, Bang. Telat lu!');
            if (!session) session = conn.ulartangga[m.chat] = new SnakeAndLadderGame(conn);
            
            if (session.players.includes(m.sender)) return m.reply('Lu udah join, sabar nape.');
            if (session.players.length >= 2) return m.reply('❌ Room penuh (Maks 2 orang).');
        ...
        case 'start':
            if (!session || session.players.length < 2) return m.reply(`Pemain kurang! Minimal 2 orang. Ketik *${usedPrefix + command} join*`);
            if (session.started) return m.reply('Game udah jalan.');
            await session.startGame(m);
            break;

        case 'roll':
            if (!session || !session.started) return m.reply('Mulai dulu gamenya, Bang.');
            const currentPlayer = session.players[session.currentPlayerIndex];
            
            if (m.sender !== currentPlayer) {
                return m.reply(`🕒 Bukan giliran lu! Sekarang jatahnya @${currentPlayer.split('@')[0]}`, null, { mentions: [currentPlayer] });
            }
            ...
            if (session.currentPositions[currentPlayer] >= session.boardSize) {
                await m.reply(`🎉 *CONGRATS!* @${currentPlayer.split('@')[0]} MENANG! Game Selesai.`, null, { mentions: [currentPlayer] });
            ...
            if (dice !== 6) {
                ...
                await m.reply('🎲 *Hoki!* Dapet angka 6, jalan lagi lu!');
            }
        ...
        case 'reset':
            delete conn.ulartangga[m.chat];
            m.reply('🔄 Sesi Ular Tangga berhasil direset!');
            break;
