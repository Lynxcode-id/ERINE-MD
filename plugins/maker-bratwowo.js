...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text) return m.reply(`Contoh: ${usedPrefix + command} orang desa gakai dolar kok yahahaha 🗿`)

    await m.react('⏳')

    try {
        const apiUrl = `https://api-nanzz.my.id/docs/api/maker/brat/brat-prabowo.php?text=${encodeURIComponent(text)}`
        ...
        console.error('[BRAT PRABOWO ERROR]', e)
        await m.react('❌')
        m.reply(`❌ Gagal membuat stiker.\n> *Detail:* ${e.message}`)
    }
}

handler.help = ['bratprabowo <teks>']
handler.tags = ['maker']
handler.command = /^(bratprabowo|bratwowo|wowobrat)$/i
handler.limit = true
