...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    await m.reply('⏳ *Mendeteksi tingkat kemageran...*');

    try {
        let apiUrl = `https://api.jagoanproject.biz.id/api/fun/cekmager?name=${encodeURIComponent(name)}`;
...
handler.help = ['cekmager <nama/tag>'];
handler.tags = ['fun'];
handler.command = /^(cekmager|mager)$/i; 
handler.limit = true;
