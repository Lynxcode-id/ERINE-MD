...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text) return m.reply(`Contoh: ${usedPrefix + command} Halo semuanya namaku nanas`)

    await m.react('⏳')

    try {
        const apiUrl = `https://api-nanzz.my.id/docs/api/maker/brat-patrick.php?text=${encodeURIComponent(text)}`
        ...
        console.error('[BRAT PATRICK ERROR]', e)
        await m.react('❌')
        m.reply(`❌ Gagal membuat stiker.\n> *Detail:* ${e.message}`)
    }
}

handler.help = ['bratpatrick <teks>']
handler.tags = ['maker']
handler.command = /^(bratpatrick|patrick)$/i
handler.limit = true
