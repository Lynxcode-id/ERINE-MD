...
let handler = async (m, { text, usedPrefix, command }) => {
    ...
    // Pastikan foldernya ada, biar ga error pas ngelist kalo ternyata belom pernah bikin
    if (!fs.existsSync(scrapeDir)) fs.mkdirSync(scrapeDir, { recursive: true });

    // Deteksi command apa yang lagi dipake
    ...
    if (isList) {
        try {
            ...
            // Filter cuma tampilin file .js aja biar rapi
    ...
    if (isDel) {
        ...
        // Otomatis tambahin .js kalo lu lupa ngetik ekstensi
...
handler.tags = ['owner'];
// lsp = List Scrape, dsp = Delete Scrape
