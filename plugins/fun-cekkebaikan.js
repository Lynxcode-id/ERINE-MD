...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    await m.reply('⏳ *Mendeteksi tingkat kebaikan...*');

    try {
        let apiUrl = `https://api.jagoanproject.biz.id/api/fun/cekkebaikan?name=${encodeURIComponent(name)}`;
...
handler.help = ['cekkebaikan <nama/tag>'];
handler.tags = ['fun'];
handler.command = /^(cekkebaikan|kebaikan)$/i; 
handler.limit = true;
