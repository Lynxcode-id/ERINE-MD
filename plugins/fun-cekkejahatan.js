...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    await m.reply('⏳ *Mendeteksi tingkat kejahatan...*');

    try {
        let apiUrl = `https://api.jagoanproject.biz.id/api/fun/cekkejahatan?name=${encodeURIComponent(name)}`;
...
handler.help = ['cekkejahatan <nama/tag>'];
handler.tags = ['fun'];
handler.command = /^(cekkejahatan|kejahatan)$/i; 
handler.limit = true;
