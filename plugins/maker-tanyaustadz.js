 ...
 * ─────────────────────────
 * 📝 Plugin: Maker Tanya Ustadz (Xemoz API)
 */
...
let handler = async (m, { conn, text, prefix, command }) => {
    if (!text) {
        return m.reply(`⚠️ *Format Salah!*\n\n*Contoh:* ${prefix + command} apa hukum nikah bini muda`);
    }

    await m.react('⚡');

    try {
        let apiUrl = `https://api-xemoz-official.my.id/api/maker/tanya-ustadz.php?text=${encodeURIComponent(text)}`;
        ...
        if (contentType.includes('image')) {
            ...
            // Fix ngambil dari json.result.image sesuai log lu cuy
            let mediaData = json.result?.image || json.result?.url || json.url;
            
            if (!mediaData) {
                throw new Error(`Struktur JSON berubah: ${JSON.stringify(json).substring(0, 100)}...`);
            }
...
handler.help = ['tanyaustadz <teks>', 'ustadz <teks>'];
handler.tags = ['maker'];
handler.command = /^(tanyaustadz|ustadz|tanyaustad)$/i;
handler.limit = true;
