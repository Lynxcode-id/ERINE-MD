 ...
 * >> 📞 Contact Author : +62 882-5804-1396
 * >> 📢 Official Channels : https://whatsapp.com/channel/0029VbAnuii6GcGCu73oep1i
 * >> ⚠️ Note      : Keep credit to respect the creator!
 * >> 📝 Plugin      : iOS Quote Creator (IQC)
 */
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (!txt) return m.reply(`⚠️ Masukkan teks atau balas pesan!\n> *Contoh:* ${usedPrefix + command} Halo namaku nanas 🤪`);
    ...
    try {
        ...
        const [emojiPrimaryRes, emojiAppleRes, bgRes, fontRes] = await Promise.all([
            fetch(EMOJI_URLS[brand]),
            brand !== 'apple' ? fetch(EMOJI_URLS.apple) : null,
            fetch(BG_URL),
            fetch(FONT_URL),
        ]);

        const emojiPrimary = await emojiPrimaryRes.json();
        const emojiApple = emojiAppleRes ? await emojiAppleRes.json() : null;
        ...
        await conn.sendMessage(m.chat, { 
            image: finalBuffer, 
            caption: `✅ Berhasil membuat Fake iOS Quote.\n> © Erine-MD` 
        }, { quoted: m });

        await m.react('🔥');

    } catch (e) {
        console.error('[iqc error tod]', e);
        await m.react('😨');
...
handler.tags = ['maker'];
handler.command = /^(iqc4|fakeiqc4|iosquote4)$/i;
handler.limit = true;
