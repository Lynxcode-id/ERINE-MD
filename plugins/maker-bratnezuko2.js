 ...
 * ─────────────────────────
 * 📝 Plugin: Brat Nezuko Sticker Maker
 */
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (!txt) {
        return m.reply(`┌˚₊ ๑│ ʙ ʀ ᴀ ᴛ  ɴ ᴇ ᴢ ᴜ ᴋ ᴏ │๑˚₊ ⚠️\n┇ \n│ ❌ *Teks kosong!*\n│ \n│ 📌 *Cara pakai:*\n│ ${usedPrefix + command} <teks>\n│ _Atau reply pesan teks yang ingin dijadikan stiker._\n┇ \n└˚₊ ๑ ────────────── ๑˚₊\n> © ERINE-AI`)
    }

    await m.react('⏳')

    try {
        const apiUrl = `https://restapi.jagoanproject.web.id/api/maker/bratnezuko?text=${encodeURIComponent(txt)}`
        ...
        console.error('[BRATNEZUKO ERROR]', error)
        await m.react('❌')
...
handler.help = ['bratnezuko2 <teks>']
handler.tags = ['maker']
handler.command = /^(bratnezuko2)$/i
handler.limit = true
