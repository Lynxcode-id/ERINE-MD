 ...
 * ─────────────────────────
 * 📝 Plugin : HD Video Enhancer (Integrated)
 */
...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (!mime.startsWith('video/')) {
        return m.reply(`┌˚₊ ๑│ ʜ ᴅ  ᴠ ɪ ᴅ ᴇ ᴏ │๑˚₊ 🎥\n┇ \n│ ❌ Reply atau kirim video yang mau di-HD-in cuy!\n│ \n│ *Cara Pakai:*\n│ ${usedPrefix + command} (sambil reply video)\n┇ \n└˚₊ ๑ ────────────── ๑˚₊\n> © ERINE-AI`);
    }

    await m.react('⏳');
    m.reply('⏳ *Processing...*\nVideo sedang di-HD-kan. Proses ini memakan waktu beberapa menit tergantung ukuran video dan antrean server.');

    try {
        let media = await q.download();
        let res = await videoenhancer(media);

        if (!res || !res.resultUrl) {
            throw new Error("Gagal mendapatkan URL hasil render HD dari server.");
        }

        let txt = `┌˚₊ ๑│ ᴠ ɪ ᴅ ᴇ ᴏ  ᴇ ɴ ʜ ᴀ ɴ ᴄ ᴇ ʀ │๑˚₊ ✨\n┇ \n│ ✅ *Video berhasil di-HD-kan!*\n│ ⏱️ *Waktu Proses:* Selesai\n┇ \n└˚₊ ๑ ────────────── ๑˚₊\n> © ERINE-AI`;

        await conn.sendMessage(m.chat, { 
            video: { url: res.resultUrl }, 
            caption: txt 
        }, { quoted: m });
        ...
        console.error('[HD VIDEO ERROR]', e);
        await m.react('❌');
        m.reply(`┌˚₊ ๑│ s ʏ s ᴛ ᴇ ʀ ᴍ  ᴇ ʀ ʀ ᴏ ʀ │๑˚₊ ❌\n┇ Gagal memproses video:\n┇ ${e.message}\n└˚₊ ๑ ────────────── ๑˚₊`);
    }
}
