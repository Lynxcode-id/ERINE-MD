...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    // Fake Reply Kontak Global khas Jemima
    ...
    try {
        ...
        // Handling response sesuai dengan output API masing-masing
        if (command === 'tohijab' && json.status) {
            resultUrl = json.result.hijab
        } else if (command === 'tomessybun' && json.success) {
            resultUrl = json.result
        } else {
            throw 'API gagal merespon atau hasil tidak ditemukan.'
        }

        let caption = `╭─⟡ *A I  S T Y L E  E D I T O R* ⟡─╮
│ 👤 *Mode* : ${command === 'tohijab' ? 'Hijab Style' : 'Messy Bun Style'}
│ ✨ *Status* : Berhasil
╰────────────────────⟡

_✦ ᴊᴇᴍɪᴍᴀ ᴍᴜʟᴛɪ ᴅᴇᴠɪᴄᴇ ✦_`
