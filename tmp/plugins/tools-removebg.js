...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        // 4. Upload menggunakan lib uploadImage bawaan Jemima/Erine
        let fileUrl = await uploadImage(imgBuffer)

        if (!fileUrl || !/^https?:\/\//i.test(fileUrl)) {
            await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } })
            return m.reply('⚠️ *Gagal mengupload gambar ke server sementara. Coba lagi nanti.*')
        }
