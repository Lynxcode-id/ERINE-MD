...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  // 🔥 PILAN ANTI-CRASH: Pake gambar default kalau profilePicture null/kosong
  let validImage = account.profilePicture && account.profilePicture.startsWith('http') 
  ...
  try {
    ...
    const getPresence = id => axios.post('https://presence.roblox.com/v1/presence/users', { userIds: [id] }, { headers }).then(res => {
      ...
      return {
        ...
        recentGame: p.lastLocation || 'undefined'
      }
    }).catch(() => ({ isOnline: false, lastOnline: 'Tidak tersedia', location: '❌ Tidak sedang bermain apa pun (offline)', recentGame: 'undefined' }))
