...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    /**
     * @type {import('@wishkeysocket/baileys').WASocket | import('@wishkeysocket/baileys').WALegacySocket}
     */
    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendFile: {
             ...
             * @param {import('@wishkeysocket/baileys').proto.WebMessageInfo} quoted
             * @param {Boolean} ptt
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                /**
                 * @type {import('@wishkeysocket/baileys').proto.WebMessageInfo}
                 */
                let m
                try {
...
END:VCARD`.trim()
        ...
        reply: {
            ...
            value(jid, text = '', quoted, options) {
                // let pp = conn.profilePictureUrl(conn.user.jid, 'image') // Note: variabel pp ini sepertinya tidak terpakai
                return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, {
                    text,
                    mentions: conn.parseMention(text),
                    contextInfo: global.adReply?.contextInfo || {}, // <-- FIX: Tambahkan ?. dan fallback object kosong
                    ...options
                }, {
                    quoted,
   ...
   */
    ...
    */
     ...
     */
        ...
        cMod: {
             ...
             * cMod
             * @param {String} jid 
             * @param {import('@wishkeysocket/baileys').proto.WebMessageInfo} message 
             * @param {String} text 
             * @param {String} sender 
             * @param {*} options 
             * @returns 
        ...
        loadMessage: {
             ...
             * @returns {import('@wishkeysocket/baileys').proto.WebMessageInfo}
             */
            value(messageID) {
        ...
        processMessageStubType: {
             ...
             * @param {import('@wishkeysocket/baileys').proto.WebMessageInfo} m 
             */
            async value(m) {
            ...
            async value(m) {
                ...
                for (const message of m) {
                    try {
                        ...
                        // 🔥 FIX: Tambahin opsional chaining (?.) biar kebal dari pesan sistem tanpa identitas
                        const chat = conn.decodeJid(message.key?.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
                        
                        if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
                            /**
                             * @type {import('@wishkeysocket/baileys').proto.IContextInfo}
                             */
                            let context = message.message[mtype].contextInfo
                            let participant = conn.decodeJid(context.participant)
                            const remoteJid = conn.decodeJid(context.remoteJid || participant)
                            /**
                             * @type {import('@wishkeysocket/baileys').proto.IMessage}
                             * */
                        ...
                        if (isGroup) {
                            ...
                            // 🔥 FIX: Amanin dari error undefined
                        ...
                        // 🔥 FIX: Amanin cek fromMe dari error undefined
                        const fromMe = message.key?.fromMe || areJidsSameUser(sender || chat, conn.user.id)
                        if (!['protocolMessage'].includes(mtype) && !fromMe && message.messageStubType != WAMessageStubType.CIPHERTEXT && message.message) {
                            delete message.message.messageContextInfo
                            delete message.message.senderKeyDistributionMessage
                            
                            // 🔥 FIX: Hindari save object pakai key kosong
                            if (message.key?.id) {
    ...
    */
    ...
    // 🔥 FIX: Nanggulangi LID bocor di Jadibot tetep nyala
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        isBaileys: {
            get() {
                // 🔥 FIX: Nambahin regex prefix 'BAE5' buat versi Baileys baru 
                return this.id?.length === 16 || (this.id?.startsWith('3EB0') || this.id?.startsWith('BAE5')) && (this.id?.length === 22 || this.id?.length === 16) || false
            }
        },
        chat: {
            get() {
        ...
        text: {
            get() {
                ...
                // 🔥 FIX: Nangkep balasan button jadibot (Interactive / List / Template / Button)
                let btnRes = '';
                if (this.mtype === 'interactiveResponseMessage') {
                    try {
                        let params = JSON.parse(msg?.nativeFlowResponseMessage?.paramsJson || '{}');
                        btnRes = params.id || ''; 
                    } catch (e) {}
                } else if (this.mtype === 'templateButtonReplyMessage') {
                    btnRes = msg?.selectedId || '';
                } else if (this.mtype === 'buttonsResponseMessage') {
                    btnRes = msg?.selectedButtonId || '';
                } else if (this.mtype === 'listResponseMessage') {
                    btnRes = msg?.singleSelectReply?.selectedRowId || '';
                }

                const text = (typeof msg === 'string' ? msg : msg?.text) || msg?.caption || msg?.contentText || btnRes || '';
        ...
        isBot: {
            get() {
                ...
                // 🔥 FIX: Nambahin regex BAE5
                return ["3EB", "BAE5"].some(k => idBot.includes(k) && this.sender !== this.conn?.user.jid);
            },
            enumerable: true
        }
    })
