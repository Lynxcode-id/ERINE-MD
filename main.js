...
// --- FIX IMPORT BAILEYS V7 & ADD CACHE ---
...
const connectionOptions = {
  ...
  msgRetryCounterCache, // Added for v7 stability
  cachedGroupMetadata: async (jid) => groupCache.get(jid), // Added for v7 group cache
...
function clearTmp() {
  ...
  const filename = []
  tmp.forEach(dirname => {
    if(existsSync(dirname)) readdirSync(dirname).forEach(file => filename.push(join(dirname, file)))
  })
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    ...
    // CACHE METADATA V7 EVENTS BINDING
    conn.ev.on('groups.update', async ([event]) => {
        if(event && event.id) {
            const metadata = await conn.groupMetadata(event.id).catch(() => null)
    ...
    try {
      const inviteCode = '120363424957729073@g.us' 
    ...
    try {
      ...
      console.log(chalk.green('✅ Erine sudah, Siap menerima perintah'))
    } catch (e) {
      console.log(chalk.red('❌ Sepertinya ada yang salah', e))
    }
