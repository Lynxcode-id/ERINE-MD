    ...
    let sock = Object.defineProperties(conn, {
        ...
        getJid: {
            value(sender) {
                ...
                if (conn.isLid[s]) {
                    let cached = conn.isLid[s]
                    if (/^\d+$/.test(cached)) cached = `${cached}@s.whatsapp.net`
                    return cached
                }
                ...
                for (let chat of Object.values(conn.chats)) {
                    ...
                    if (user) {
                        let resolved = (user.phoneNumber ? `${String(user.phoneNumber).replace(/[^0-9]/g, '')}@s.whatsapp.net` : null) 
                            || (user.id && !user.id.endsWith('@lid') ? user.id : null)
                            || (user.jid && !user.jid.endsWith('@lid') ? user.jid : null);

                        if (resolved) {
                            resolved = resolved.decodeJid ? resolved.decodeJid() : String(resolved).decodeJid()
                            if (/^\d+$/.test(resolved)) resolved = `${resolved}@s.whatsapp.net`
                            conn.isLid[s] = resolved
                            return conn.isLid[s]
                        }
                    }
                }
                        ...
                        try {
                            ...
                            if (param) {
                                ...
                                if (parsed.id && parsed.phoneNumber) {
                                    if (!conn.isLid) conn.isLid = {}
                                    let phoneNum = String(parsed.phoneNumber).replace(/[^0-9]/g, '')
                                    conn.isLid[parsed.id] = `${phoneNum}@s.whatsapp.net`
                                }
                            }
                        } catch (e) {}
                        break
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                if (this.key?.fromMe) return (this.conn?.user?.jid || this.conn?.user?.id || '').decodeJid()
                const candidates = [
...
async function generateProfilePicture(mediaUpload) {
    ...
    try {
        ...
        if (!Jimp) throw new Error('Module Jimp bener-bener kosong njir.')
        ...
        if (typeof Jimp.read === 'function') {
            ...
            readImage = await jimpModule.read(bufferOrFilePath)
        } else {
            throw new Error('Gak nemu fungsi read di Jimp lu.')
        }
        ...
        const mime = Jimp.MIME_JPEG || jimpModule.MIME_JPEG || 'image/jpeg'
        const auto = Jimp.AUTO || jimpModule.AUTO || -1
