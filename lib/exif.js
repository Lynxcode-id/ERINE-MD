...
async function imageToWebp(media) {
    ...
    try {
        ...
        console.error('[EXIF] Error imageToWebp:', error);
        throw error;
    } finally {
...
async function videoToWebp(media) {
    ...
    try {
        ...
        console.error('[EXIF] Error videoToWebp:', error);
        throw error;
    } finally {
        if (fs.existsSync(tmpFileIn)) await fs.promises.unlink(tmpFileIn).catch(() => {});
        if (fs.existsSync(tmpFileOut)) await fs.promises.unlink(tmpFileOut).catch(() => {});
...
async function writeExifImg(media, metadata) {
    ...
    try {
        ...
        console.error('[EXIF] Error writeExifImg:', error);
        return wMedia;
    } finally {
        if (fs.existsSync(tmpFileIn)) await fs.promises.unlink(tmpFileIn).catch(() => {});
        if (fs.existsSync(tmpFileOut)) await fs.promises.unlink(tmpFileOut).catch(() => {});
...
async function writeExifVid(media, metadata) {
    ...
    try {
        ...
        console.error('[EXIF] Error writeExifVid:', error);
        return wMedia;
    } finally {
        if (fs.existsSync(tmpFileIn)) await fs.promises.unlink(tmpFileIn).catch(() => {});
        if (fs.existsSync(tmpFileOut)) await fs.promises.unlink(tmpFileOut).catch(() => {});
