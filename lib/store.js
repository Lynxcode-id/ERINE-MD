...
function bind(conn) {
    ...
    const resolveJid = (id) => {
        if (!id) return id;
        let decoded = conn.decodeJid(id);
        if (decoded && decoded.endsWith('@lid') && typeof conn.getJid === 'function') {
            return conn.getJid(decoded);
        }
        return decoded;
    };
    ...
    function updateNameToDb(contacts) {
        ...
        try {
            ...
            for (const contact of contacts) {
                ...
                const id = resolveJid(contact.id)
    ...
    conn.ev.on('presence.update', async function presenceUpdatePushToDb({ id, presences }) {
        try {
            ...
            const _sender = resolveJid(sender)
            ...
            const decodedGroup = resolveJid(id)
            if (decodedGroup.endsWith('@g.us')) {
