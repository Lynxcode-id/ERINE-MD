/**
 * @param {import('@whiskeysocket/baileys').WASocket | import('@whiskeysocket/baileys').WALegacySocket} conn
 */
function bind(conn) {
    if (!conn.chats) conn.chats = {}
    
    /**
     * @param {import('@whiskeysocket/baileys').Contact[]|{contacts:import('@whiskeysocket/baileys').Contact[]}} contacts 
     * @returns 
     */
    function updateNameToDb(contacts) {
        ...
        try {
            ...
            for (const contact of contacts) {
                if (!contact.id) continue // Guard tambahan
    ...
    conn.ev.on('group-participants.update', async function updateParticipantsToDb({ id, participants, action }) {
        try { // Bungkus pake try-catch biar gak crash system-wide
            if (!id) return
            id = conn.decodeJid(id)
            ...
            // Guard: Pastiin fungsi groupMetadata udah ke-load sama Baileys sebelum dipanggil
    ...
    conn.ev.on('chats.upsert', function chatsUpsertPushToDb(chatsUpsert) {
        try {
            ...
            // 🔥 FIX: Safety check biar gak crash kalau prototype function ini belum ke-load
    ...
    conn.ev.on('presence.update', async function presenceUpdatePushToDb({ id, presences }) {
        try {
            ...
            // 🔥 FIX: Optional chaining biar nggak crash kalo presences[sender] tiba-tiba kosong/undefined dari server
            ...
            if (id.endsWith('@g.us')) {
                ...
                if (!groupChats && typeof conn.groupMetadata === 'function') {
                    const metadata = await conn.groupMetadata(id).catch(_ => null)
                    if (metadata) conn.chats[id] = { id, subject: metadata.subject, metadata, isChats: true }
                }
            }
        } catch (e) {
