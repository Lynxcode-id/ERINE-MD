...
class Image {
  ...
  static muxAnim({
    ...
    return _asyncToGenerator(function* () {
      ...
      // 🔥 FIX: Selalu amanin penulisan file pake try-finally
      try {
        for (let i = 0, l = out.length; i < l; i++) {
          yield fs.write(fp, out[i], 0, undefined, undefined);
        }
      } finally {
        if (fp) yield fs.close(fp).catch(() => {});
      }
    })();
  }
}
...
var _demuxFrame2 = /*#__PURE__*/ (function () {
  var _demuxFrame3 = _asyncToGenerator(function* (path, frame) {
    ...
    // 🔥 FIX: Sama kayak di atas, tutup file apa pun yang terjadi
    try {
      for (let i = 0, l = out.length; i < l; i++) {
        yield fs.write(fp, out[i], 0, undefined, undefined);
      }
...
var _read2 = /*#__PURE__*/ (function () {
  var _read3 = _asyncToGenerator(function* (path) {
    ...
    // 🔥 FIX UTAMA: Bungkus semua proses read ke dalam try-finally
    // Ini biar file descriptor (fd) pasti ditutup sebelum function ini selesai.
    try {
